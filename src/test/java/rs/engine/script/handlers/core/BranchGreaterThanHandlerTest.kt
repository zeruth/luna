...
class BranchGreaterThanHandlerTest {

    private val handler = BranchGreaterThanHandler()

    private val opcode = RuneScriptOpcode.BRANCH_GREATER_THAN

    @Test
    fun `pc increments when a is greater than b`() {
    ...
    @Test
    fun `pc does not increment when a equals b`() {
        val script = FakeScriptFile.simple(
            opcodes = intArrayOf(opcode),
            intOperands = intArrayOf(3)
        )
